...
public class JVMMemoryTest {
    ...
    public static void main() {
         ...
         *
         * 2.创建Student的时候，首先进行类的加载工作，类只会加载一次，将类的类型信息数据加载到jvm的方法区中，
         * 如果之前加载过了，那么就不会再加载。
         *
         * 3.类的加载其实就是将.class文件加载进虚拟机内存中，在加载的时候，在java堆中生成对应的Class对象，
         * 最后生成一个Student对象在堆中
         * 
         * 4.new 关键字后面是类的构造函数，构造函数也是类的静态方法， 所以会促成类的加载，如果类是第一次加载，
         * 那么在加载的过程会执行静态代码块，所有加载流程已经完成了，才真正执行构造方法，所以new一个对象的时候，
         * 类的静态代码块会执行在先，然后才会执行到构造方法。
         */
